...
function createTextTexture(gl: any, text: string, font = 'bold 30px monospace', color = 'white') {
  ...
  const textWidth = Math.max(Math.ceil(metrics.width), 50);
  const textHeight = Math.max(Math.ceil(getFontSize(font) * 1.4), 40);
...
function createPlaceholderCanvas(text: string) {
  const canvas = document.createElement('canvas');
  canvas.width = 800;
  canvas.height = 600;
  ...
  if (ctx) {
    ...
    ctx.fillText(text || 'Skill', 400, 300);
  }
  return canvas;
}
...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          float imgAspect = (uImageSizes.x > 0.0 && uImageSizes.y > 0.0) ? (uImageSizes.x / uImageSizes.y) : 1.333;
          float planeAspect = (uPlaneSizes.x > 0.0 && uPlaneSizes.y > 0.0) ? (uPlaneSizes.x / uPlaneSizes.y) : 1.333;
          vec2 ratio = vec2(
            min(planeAspect / imgAspect, 1.0),
            min((1.0 / planeAspect) / (1.0 / imgAspect), 1.0)
          );
          vec2 uv = vec2(
    ...
    const setFallback = () => {
      const fallbackCanvas = createPlaceholderCanvas(this.text);
  ...
  update(scroll: any, direction: string) {
    ...
    if (this.bend === 0) {
      ...
      const arc = R - Math.sqrt(Math.max(R * R - effectiveX * effectiveX, 0.001));
      if (this.bend > 0) {
        this.plane.position.y = -arc;
        this.plane.rotation.z = -Math.sign(x) * Math.asin(Math.min(effectiveX / R, 1.0));
      } else {
        this.plane.position.y = arc;
        this.plane.rotation.z = Math.sign(x) * Math.asin(Math.min(effectiveX / R, 1.0));
      }
  ...
  onResize({ screen, viewport }: any = {}) {
    ...
    const screenWidth = this.screen?.width || 800;
    const screenHeight = this.screen?.height || 600;
...
class AppGallery {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      { image: `https://picsum.photos/seed/1/800/600?grayscale`, text: 'React' },
      { image: `https://picsum.photos/seed/2/800/600?grayscale`, text: 'Next.js' },
      { image: `https://picsum.photos/seed/3/800/600?grayscale`, text: 'TypeScript' },
      { image: `https://picsum.photos/seed/4/800/600?grayscale`, text: 'Node.js' },
      { image: `https://picsum.photos/seed/5/800/600?grayscale`, text: 'Python' },
      { image: `https://picsum.photos/seed/16/800/600?grayscale`, text: 'Docker' },
    ];
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        console.warn('CircularGallery initialization warning:', err);
      }
    };
